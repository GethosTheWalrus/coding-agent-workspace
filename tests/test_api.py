...
class TestCreateTodoEndpoint:
    ...
    def test_create_todo_with_completed_true(self, client: TestClient):
        ...
        response = client.post(
            "/todos", json={"title": "Completed Todo", "completed": True}
        )
...
class TestListTodosEndpoint:
    ...
    def test_list_todos_with_pagination(
        self, client: TestClient, sample_todo_data: dict
    ):
    ...
    def test_list_todos_filter_by_completed(
        self, client: TestClient, sample_todo_data: dict
    ):
        """Test listing todos filtered by completion status."""
        # Create todos with different completion status
        client.post(
            "/todos", json={**sample_todo_data, "title": "Completed", "completed": True}
        )
        client.post(
            "/todos",
            json={**sample_todo_data, "title": "Not Completed", "completed": False},
        )
        client.post(
            "/todos",
            json={**sample_todo_data, "title": "Another Completed", "completed": True},
        )
